...
#define WIDTH 240
#define HEIGHT 240
const char* ssid     = "WlanOFFEN";
const char* password = "123ToLL456!!";
...
void handleRoot() {
  ...
  html += "    const pass = prompt('Passwort (optional, leerlassen für öffentlich):');";
  html += "    const roomData = {";
  html += "      info: {";
  html += "        roomName: name,";
  html += "        password: pass || null,";
  html += "        created: Date.now(),";
  html += "        createdBy: getMyName()";
  html += "      },";
  html += "      chat: {}";
  html += "    };";
  html += "    const ref = db.ref(GLOBAL_REF + '/parties').push();";
  html += "    ref.set(roomData).then(() => {";
  html += "      debug('Room created: ' + name + ' (ID: ' + ref.key + ')');";
  html += "      joinRoom(ref.key, name);";
  html += "    }).catch(e => {";
  html += "      debug('ROOM CREATE ERROR: ' + e.message);";
  html += "    });";
  html += "  } catch(e) {";
  html += "    debug('CREATE ROOM ERROR: ' + e.message);";
  html += "  }";
  html += "}";
  html += "function joinRoom(id, name) {";
  html += "  try {";
  html += "    debug('Joining room: ' + name + ' (' + id + ')');";
  html += "    db.ref(GLOBAL_REF + '/parties/' + id + '/info/password').once('value', snap => {";
  html += "      const pass = snap.val();";
  html += "      if(pass && prompt('Passwort erforderlich:') !== pass) {";
  html += "        alert('ZUTRITT VERWEIGERT!');";
  html += "        return;";
  html += "      }";
  ...
  html += "    });";
  html += "  } catch(e) {";
  html += "    debug('JOIN ERROR: ' + e.message);";
  html += "  }";
  html += "}";
  html += "function leaveToGlobal() {";
  html += "  try {";
  html += "    debug('Returning to global...');";
  html += "    if(chatListener) chatListener.off();";
  ...
  html += "    listenToChat();";
  html += "  } catch(e) {";
  html += "    debug('LEAVE ERROR: ' + e.message);";
  html += "  }";
  html += "}";
...
void handleCmd() {
  ...
  else if (lowerCmd.startsWith("speed ")) { int newSpeed = cmd.substring(6).toInt(); if (newSpeed > 0) { cosmosSpeed = newSpeed; terminalLog = "WARP FACTOR: " + String(newSpeed); }
} 
...
void drawStarfield() {
  for(int i = 0; i < 100; i++) { int prev_z = stars[i].z; int prev_px = 120 + (stars[i].x * 128) / prev_z; int prev_py = 120 + (stars[i].y * 128) / prev_z; stars[i].z -= cosmosSpeed;
    if(stars[i].z <= 0) { stars[i].z = 1000; stars[i].x = random(-500, 500); stars[i].y = random(-500, 500); continue; }
    int px = 120 + (stars[i].x * 128) / stars[i].z; int py = 120 + (stars[i].y * 128) / stars[i].z;
    if(px >= 0 && px < 240 && py >= 0 && py < 240) { uint16_t color = 0x4208;
      if(stars[i].z < 300) { 
        color = 0xFFFF; 
      } 
